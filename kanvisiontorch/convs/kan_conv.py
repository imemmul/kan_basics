from kans import KAN, KANLayer, FastKAN
import time
...
# TODO so BatchNorm needed to grad update spline_weights, i guess they explode while feedforwarded.
...
class Conv2DFastKAN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, device='cpu'):
        ...
        self.attention_maps = []
        for i in range(out_channels):
            kan_networks = FastKAN([in_channels * kernel_size * kernel_size, in_channels * kernel_size * kernel_size]).to(self.device)
            self.kan_layers.append(kan_networks)
        self.bn = nn.BatchNorm2d(out_channels).to(self.device)
    def forward(self, x):
        x = x.to(self.device)
