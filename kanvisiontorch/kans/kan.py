...
class KANLayer(nn.Module):
    def __init__(self, n_in, n_out, base_activation=nn.GELU, x_bounds=[-1, 1], k=3, device='cpu', **kwargs):
        ...
        self.device = device
        self.base_activation = base_activation()
        # self.grid_size = self.n_out - k + 1  # Ensure this calculation is correct
        self.grid_size = 5
...
#         nn.init.kaiming_uniform_(self.spline_weights, nonlinearity='relu')
#         print(f"weights: {self.weights}, spline_weights: {self.spline_weights}")
#     def forward(self, x):
#         grid_values = self.grid_values.to(x.device)
#         b_out = F.linear(self.base_activation(x), self.weights)
